              ...
              <Draggable key={file} draggableId={file} index={index}>
                {(provided: any) => (
                  ...
                  >
                    ...
                    >
                      <div className='col-xl-3'>
                        <img
                          ...
                          src={`${API_BASE_URL}/${id}/${item.thumbnail as string}`}
                          alt={file}
                          style={{height: '45px'}}
                        />
                      </div>
                    ...
                    {(item.frameCompletion as number) < 100 && (
                      <>
                        <div className='position-abolute' style={{top: '50%', left: '20px'}}>
                          Uploading ... {Math.floor(item.frameCompletion as number)} %
                        </div>
                        <div
                          className='position-absolute w-75'
                          style={{left: '5px', top: '10px', right: '5px'}}
                        >
                          <div className='h-5px mx-3 w-100 bg-light mb-3'>
                            <div
                              className='bg-primary rounded h-5px'
                              role='progressbar'
                              style={{width: `${item.frameCompletion}%`}}
                            ></div>
                          </div>
                        </div>
                      </>
        ...
        {(provided: any) => (
          <div className='videos' key={provided}>
            <div className='row g-4' {...provided.droppableProps} ref={provided.innerRef}>
              <p className='mb-0 mt-1'> Upload Video </p>
